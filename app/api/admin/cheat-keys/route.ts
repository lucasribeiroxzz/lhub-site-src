...
export async function GET() {
    try {
        ...
        const allKeys = getAllCheatKeys();
        const stats = getCheatKeyStats();

        return NextResponse.json({
            success: true,
            data: {
                stats,
                planNames: CHEAT_PLAN_NAMES,
                planPrices: CHEAT_PLAN_PRICES,
                keys: allKeys.map(k => ({
                    ...k,

                    key: k.sold ? k.key.substring(0, 8) + '...' : k.key,
                    planName: CHEAT_PLAN_NAMES[k.planType]
                }))
            }
        });

    } catch (error: any) {
        console.error('[Cheat Keys GET] Erro:', error);
...
export async function POST(req: Request) {
    try {
        ...
        const result = addCheatKeys(keys, planType, sessionPayload.email as string);

        return NextResponse.json({
            success: true,
            message: `${result.added} key(s) adicionada(s) com sucesso para o plano ${CHEAT_PLAN_NAMES[planType]}!`,
            data: {
                added: result.added,
        ...
        console.error('[Cheat Keys POST] Erro:', error);
        return NextResponse.json({ success: false, message: 'Erro interno' }, { status: 500 });
    }
}
...
export async function DELETE(req: Request) {
    try {
        ...
        const deleted = deleteCheatKey(keyId);
        ...
        console.error('[Cheat Keys DELETE] Erro:', error);
        return NextResponse.json({ success: false, message: 'Erro interno' }, { status: 500 });
    }
}
